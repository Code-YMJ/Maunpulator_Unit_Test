...
class jeus_vision():
    ...
    def init_camera(self):
        ...
        self.config.enable_stream(rs.stream.depth, 640, 640, rs.format.z16, 15)
        self.config.enable_stream(rs.stream.color, 640, 640, rs.format.bgr8, 15)
    ...
    def activate(self, target):
        ...
        while is_done == False:
            ...
            result = detect(self.device, self.model, img_color, self.names, self.colors, target,
                            imgsz=list(img_color.shape[0:2]), conf_thres=0.3, iou_thres=0.3, isVisualized=self.show_cv2)
            if result != None:

                # Intrinsics & Extrinsics
                depth_intrin = depth_frame.profile.as_video_stream_profile().intrinsics
                color_intrin = color_frame.profile.as_video_stream_profile().intrinsics
                depth_to_color_extrin = depth_frame.profile.get_extrinsics_to(self.color_frame.profile)
            ...
            elif time.time_ns() - start_time > time_out:
                is_done = True
